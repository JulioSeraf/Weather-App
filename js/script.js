...
function setPainelFeelsLike(datos, objWithExtra) {
   for (let i = 0; i < datos.time.length; i++) {
      ...
      if (todayHrs == datos.time[i].slice(11, 13) && todayDay == datos.time[i].slice(8, 10)) {
         ...
         windP.textContent = Math.round(datos.windS[i]) + " km/h";
...
function getDayWeek(datos, day) {
   ...
   for (let i = 0; i < datos.temp.length; i++) {
      ...
      ordenSemana[conDay].daysTemp.push(Math.round(datos.temp[i]));
...
function getIconImg(valor) {
   let imgTemp = "";
   switch (true) {
      case valor == 0: imgTemp = 'img/icon-sunny.webp';
         break;
      case valor == 1 || valor == 2: imgTemp = 'img/icon-partly-cloudy.webp';
         break;
      case valor == 3: imgTemp = 'img/icon-overcast.webp';
         break;
      case valor >= 45 && valor <= 50: imgTemp = 'img/icon-fog.webp';
         break;
      case (valor >= 51 && valor <= 59): imgTemp = 'img/icon-drizzle.webp';
         break;
      case ((valor >= 60 && valor <= 69) || valor >= 80 && valor <= 82): imgTemp = 'img/icon-rain.webp';
         break;
      case (valor >= 70 && valor <= 89): imgTemp = 'img/icon-snow.webp';
         break;
      case valor >= 90 && valor <= 99: imgTemp = 'img/icon-storm.webp';
         break;
